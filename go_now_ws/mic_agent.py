...
def calibrate(noise_sec=1.5, enroll_sec=4.0, margin=1.6, vad_aggr=1,
    ...
    try:
        ...
        sp = _measure_frames(enroll_sec, vad=vad)
        ...
        print(f"[speech] 캘리브레이션 완료 · 소음 {noise_rms:.0f} · 발화 {speech_rms:.0f} "
              f"· 상한 {upper:.0f} · SNR {snr:+.1f}dB", flush=True)
        if snr < 6.0:
            print("[speech] ⚠ SNR<6dB — 마이크를 더 가까이(~20cm) 권장", flush=True)
        _report_calib("done", text=enroll_text)
        return upper
    finally:
...
def speech_process_loop(utter_q, stop_evt, whisper_size, threads, min_chars):
    """utter_q → 최신 발화만 STT → KoBERT → POST."""
    from faster_whisper import WhisperModel
    print("[speech] STT(faster-whisper) 로딩...", flush=True)
    ...
    while not stop_evt.is_set():
        ...
        segments, _ = stt.transcribe(audio, language="ko", beam_size=1,
...
def main():
    ...
    if not args.no_speech:
        ...
        threads.append(threading.Thread(
            ...
            args=(utter_q, stop_evt, args.whisper_size, args.threads, args.min_chars),
            daemon=True))
        threads.append(threading.Thread(
